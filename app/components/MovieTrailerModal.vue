...
const embedUrl = computed(() => {
  try {
    ...
    if (!url) {
      console.warn('Trailer URL is empty')
      return ''
    }
    ...
    console.error('❌ Error processing trailer URL:', error)
...
const detectIframeLoad = () => {
  ...
  loadTimeout.value = setTimeout(() => {
    if (isLoading.value) {
      console.warn('⚠️ Iframe taking too long to load - assuming success')
      handleIframeLoad()
    }
  }, 5000)
  ...
  const checkIframeReady = () => {
    if (iframeRef.value && iframeRef.value.contentWindow) {
      try {
        setTimeout(() => {
          if (isLoading.value) {
            console.log('✅ Iframe appears ready (contentWindow exists)')
            handleIframeLoad()
          }
        }, 2000)
      } catch (e) {
        if (isLoading.value) {
          console.log('✅ CORS error = iframe loaded successfully')
          handleIframeLoad()
        }
      }
    }
...
watch(embedUrl, (newUrl) => {
  console.log('🎬 Final embed URL:', newUrl)
